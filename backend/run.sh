...
if [ ! -f ./db/origin_db ] || [ $ForceBackupMode == 'Y' ]
 then
     ...
     cat mi.zlb | python -c "import zlib,sys;sys.stdout.write(zlib.decompress(sys.stdin.read()))"  > mi.tar 2>&1 | tee -a log
 ...
 then
    ...
    [[ -f ./db/origin_db-journal.bak ]] && mv ./db/origin_db-journal.bak origin_db-journal
     ...
     echo $"data extraction started" 2>&1 | tee -a log
     ...
     echo $"raw extraction started" 2>&1 | tee -a log
